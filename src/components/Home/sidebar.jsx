...
const Sidebar = () => {
      ...
      eplannerEvents.exam.forEach((event) => {
        if (event.date) {
          ...
          if (dayIndex >= 0) {
            ...
            const endRow = event.endtime
              ? timeToRow(event.endtime)
              : startRow + 4
  ...
  }, [selectedDate, courseTimetableList, coursedata, eplannerEvents])
        ...
        <>
          <RemindersHeader>
            ...
            {totalPages > 1 && (
              <PaginationContainer>
                <PaginationButton
                  onClick={() =>
                    setCurrentReminderPage((prev) => Math.max(1, prev - 1))
                  }
          ...
          {currentReminders.map((reminder) => {
            ...
            if (reminder.weekdays === 'Daily') {
              if (
                reminder.date <= moment().format('YYYY-MM-DD') &&
              ...
              if (
                reminder.date <= moment().format('YYYY-MM-DD') &&
                moment().subtract(9, 'week').format('YYYY-MM-DD') <=
                  reminder.date
              ) {
                dueDate = 'This week'
              } else if (
              ...
              if (
                reminder.date === moment().add(1, 'day').format('YYYY-MM-DD')
              ) {
                dueDate = 'Tomorrow'
              }
              if (reminder.date < moment().format('YYYY-MM-DD')) {
            ...
            return (
              <ReminderItem key={reminder.id}>
                ...
                <ReminderDetails>
                  ...
                  <ReminderDue>{`Due ${dueDate} | ${dueTime}`}</ReminderDue>
                </ReminderDetails>
              </ReminderItem>
            )
          })}
          {reminderReturned === 0 && (
            <ReminderEmpty>No upcoming reminders.</ReminderEmpty>
          )}
        </>
      )
    }
      ...
      <SidebarSectionHeader>
        <LeftItem>
          {selectedDate.isSame(moment(), 'day')
            ? "Today's Schedule"
            : `${selectedDate.format('dddd')}'s Schedule`}
        </LeftItem>
        <RightGroup>
