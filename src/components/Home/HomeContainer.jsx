...
const CoursesThisSemester = () => {
  ...
  useEffect(() => {
    const fetchFavCourses = async () => {
      try {
        ...
        console.log("Fav Courses:", response.results);
      } catch (error) {
  ...
  const handleNextPage = (total) =>
    setPage((prev) => (prev + 1 < Math.ceil(total / COURSES_PER_PAGE) ? prev + 1 : prev));
  ...
  return (
  <>
    <TabsRow style={{ marginLeft: '1rem', justifyContent: 'space-between', width: '958.83px' }}>
      <TabsWrapper>
    ...
    <CoursesWrapper style={{ marginTop: '16px' }}>
      <OuterContainer style={{ height: '186.61px', flexDirection: 'column', alignItems: 'flex-start' }}>
        ...
        <div style={{ padding: '0.75rem 1rem 0 1rem' }}>
          ...
          <div style={{ color: '#b0aecd', fontSize: '0.7rem', marginTop: '0.3rem' }}>
...
const CoursesWrapper = styled.div`
  position: relative;
  width: 958.83px; /* Same as OuterContainer */
  margin-left: 1rem;
`;
