...
import { CourseContentRequest, CourseContentRequestIcon
} from 'components/CoursePage'
import { ButtonSquare, LoaderAnimation, toast
...
const CourseResourceContainer = () => {
  ...
  const [loading, setLoading
    ] = useState(false) // have the resources loaded or not?
  const [allChecked, setAllChecked
    ] = useState(true) // is selectAll used?

  const [resources, setResources
    ] = useState([]) // resources - currently available resources
  const [filteredResources, setFilteredResources
    ] = useState([]) // all the filtered resources
  const [resourceFilters, setResourceFilters
    ] = useState(['Endsems'
    ]) // which resource filters are existing in the resources?
  const [appliedResourceFilters, setAppliedResourceFilters
    ] = useState([]) // which ae currently being applied? 
  ...
  useEffect(() => {
    ...
    const fetchResources = async () => {
      try {
        ...
        setResourceFilters(filterSet) // set the total filters set
        setAppliedResourceFilters(filterSet) // default the applied filters to total filters
            } catch (error) {
  ...
  const applyCourseResourceFilter = (filterSet) => {
    ...
    const filtered = resources.filter((resource) => {
      for (let i = 0; i < resource.tags.length; i += 1) {
        if (filterSet.includes(resource.tags[i
                ])){
          return true
                }
            }
        ...
        <ButtonContainer>
          {isMobileS ? (
            <CourseContentRequestIcon
              ...
              type="resources"
              style={
            { marginRight: '0.75rem' , marginLeft: '0.75rem'
            }
        }
            />
          ) : (
